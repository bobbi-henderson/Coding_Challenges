...
function sortStringsByVowels(strings){
    let vowels = strings.map((x)=>{return x.match(/([aeiouAEIOU])*/g).sort((a,b)=>{return b.length - a.length})[0]})
   
    let key = {}
  
    strings.forEach((x,i)=>{
      key[x] = vowels[i].length
    })
  
    return strings.sort((a,b)=>{return key[b] - key[a]})  
  }